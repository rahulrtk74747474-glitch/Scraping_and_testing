    ...
    if not clean:
        ...
        for i, r in enumerate(newest, 1):
            ...
            lines += [
                ...
                f"- Stop: **{_price(r.get('stop'), cur)}**",
                f"- Target: **{_price(r.get('target'), cur)}**",
...
def write_btc_journal() -> None:
    ...
    for t in trades:
        ...
        rows.append({
            ...
            "quantity": t.get("qty", ""),
            "entry_value": _fmt_num(_num(t.get("entry")) * _num(t.get("qty")), 6),
            "exit_value": _fmt_num(_num(t.get("exit")) * _num(t.get("qty")), 6),
