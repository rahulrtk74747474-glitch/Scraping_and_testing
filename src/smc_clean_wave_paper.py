...
def initial(c):
    a=float(c['starting_capital_inr']); return {'version':1,'strategy':'SMC Clean Wave major swings','symbol':c['display_symbol'],'timeframe':c['timeframe'],'starting_capital_inr':a,'cash_inr':a,'realized_pnl_inr':0.0,'position':None,'last_processed_close_time':None,'last_signal':None,'last_sequence_direction':None,'buy_step':0,'buy_sequence_base_inr':0.0,'sell_step':0,'sell_sequence_base_qty':0.0,'equity_peak_inr':a,'max_drawdown_pct':0.0,'sell_executions':0,'wins':0,'losses':0}

def load(c,p): return json.loads(p['state'].read_text()) if p['state'].exists() else initial(c)
...
def frac(step): return .5**(int(step)+1)
def fmt(ms): return datetime.fromtimestamp(int(ms)/1000,tz=timezone.utc).isoformat() if ms else '-'

def trade(s,g,b,c,fx):
    o=[];t=[]; rate=float(c['fee_rate']); pxq=float(b.close); pxi=inr(pxq,c,fx); equity,_=eq(s,pxi); d=g['signal']; prior=s.get('last_sequence_direction')
    if d=='BUY':
        if prior!='BUY': s['buy_step']=0;s['buy_sequence_base_inr']=equity;s['sell_step']=0;s['sell_sequence_base_qty']=0.0
        st=int(s['buy_step']); f=frac(st); base=float(s['buy_sequence_base_inr'] or equity); cash=float(s['cash_inr']); n=min(base*f,cash/(1+rate))
        if n>=float(c['min_trade_inr']):
            fee=n*rate; qty=n/pxi; p=s.get('position') or {'first_entry_time':b.close_time,'qty':0.0,'cost_basis_inr':0.0}
            p['qty']=float(p['qty'])+qty;p['cost_basis_inr']=float(p['cost_basis_inr'])+n+fee;p['avg_entry_price_inr']=p['cost_basis_inr']/p['qty'];p['last_buy_time']=b.close_time;s['position']=p;s['cash_inr']=cash-n-fee;s['buy_step']=st+1
            o.append({'close_time':b.close_time,'signal':'BUY','side':'BUY','ladder_step':st+1,'fraction_pct':f*100,'sequence_base':base,'price_quote':pxq,'price_inr':pxi,'qty':qty,'gross_notional_inr':n,'fee_inr':fee,'cash_after_inr':s['cash_inr']})
    else:
        p=s.get('position')
        if prior!='SELL': s['sell_step']=0;s['sell_sequence_base_qty']=float(p['qty']) if p else 0.0;s['buy_step']=0;s['buy_sequence_base_inr']=0.0
        if p:
            st=int(s['sell_step']); f=frac(st); base=float(s['sell_sequence_base_qty'] or p['qty']); before=float(p['qty']); qty=min(base*f,before); gross=qty*pxi
            if gross>=float(c['min_trade_inr']):
                fee=gross*rate; proceeds=gross-fee; cb=float(p['cost_basis_inr']); cost=cb*qty/before; net=proceeds-cost; remq=before-qty; remc=max(0,cb-cost)
                s['cash_inr']=float(s['cash_inr'])+proceeds;s['realized_pnl_inr']=float(s['realized_pnl_inr'])+net;s['sell_executions']=int(s['sell_executions'])+1;s['wins']=int(s['wins'])+(1 if net>0 else 0);s['losses']=int(s['losses'])+(1 if net<0 else 0);s['sell_step']=st+1
                o.append({'close_time':b.close_time,'signal':'SELL','side':'SELL','ladder_step':st+1,'fraction_pct':f*100,'sequence_base':base,'price_quote':pxq,'price_inr':pxi,'qty':qty,'gross_notional_inr':gross,'fee_inr':fee,'cash_after_inr':s['cash_inr']})
                t.append({'entry_time':p['first_entry_time'],'exit_time':b.close_time,'exit_ladder_step':st+1,'fraction_pct':f*100,'exit_price_quote':pxq,'exit_price_inr':pxi,'qty':qty,'allocated_cost_inr':cost,'exit_proceeds_inr':proceeds,'net_pnl_inr':net,'return_pct':net/cost*100 if cost else 0})
                if remq<=1e-12 or remc<=1e-8:s['position']=None
                else:p['qty']=remq;p['cost_basis_inr']=remc;p['avg_entry_price_inr']=remc/remq;p['last_sell_time']=b.close_time;s['position']=p
    s['last_sequence_direction']=d;s['last_signal']=g;return o,t

def report(s,c,p,last,fx):
    px=inr(last.close,c,fx); equity,un=eq(s,px); start=float(s['starting_capital_inr']); pos=s.get('position'); g=s.get('last_signal'); direction=s.get('last_sequence_direction'); step=int(s['buy_step'] if direction=='BUY' else s['sell_step'] if direction=='SELL' else 0); nxt=frac(step)*100 if direction else 50.0
    L=[f"# {c['display_symbol']} {c['timeframe']} SMC Major-Swing Paper Trader — Latest",'',f"- Starting demo money: **₹{start:,.2f}**",f"- Current equity: **₹{equity:,.2f}**",f"- Cash: **₹{float(s['cash_inr']):,.2f}**",f"- Total P&L: **₹{equity-start:,.2f} ({(equity/start-1)*100 if start else 0:.2f}%)**",f"- Realized / unrealized: **₹{float(s['realized_pnl_inr']):,.2f} / ₹{un:,.2f}**",f"- Max drawdown: **{float(s['max_drawdown_pct']):.2f}%**",f"- Last processed candle: **{fmt(s['last_processed_close_time'])}**",f"- Next ladder fraction: **{nxt:.6f}%**",'', '## Position','']
    L += [f"- LONG qty: **{float(pos['qty']):.10f}** | cost basis: **₹{float(pos['cost_basis_inr']):,.2f}**" if pos else '_Flat. SELL while flat does not open a short._','', '## Latest confirmed major swing','']
    L += [f"- **{g['signal']}** confirmed {fmt(g['confirmed_close_time'])}; pivot candle {fmt(g['pivot_open_time'])}; pivot {float(g['pivot_price']):,.6f} {c['quote_currency']}" if g else '_None recorded since initialization._','', '## Rules','',f"- Timeframe **{c['timeframe']}**, sig_sens **{int(c['sig_sens'])}**.",'- BUY = confirmed pivot low; SELL = confirmed pivot high.','- Execute at confirmation-candle close, never on the older back-plotted pivot candle.','- BUY ladder: **50%, 25%, 12.5%, 6.25%, ...** of equity at start of the BUY sequence, capped by cash.','- SELL ladder: **50%, 25%, 12.5%, 6.25%, ...** of quantity at start of the SELL sequence, capped by remaining quantity.','- Opposite signal resets the ladder sequence.','- No historical trade replay on initialization.',f"- Data: **{c['source']} / {c['ticker']}**.",f"- Note: {c.get('source_note','')}",'','_Paper trading only; no real orders._','']
...
def main():
    ...
    addcsv(p['orders'],orders,['close_time','signal','side','ladder_step','fraction_pct','sequence_base','price_quote','price_inr','qty','gross_notional_inr','fee_inr','cash_after_inr'])
